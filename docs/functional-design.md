...
- 投稿詳細Scene内の`POST.order`は役割を兼ねる:`0`=メイン投稿、`1`=引用ポスト(任意)、`2`以上=リプライ(追加順・並び替え順を昇順で表現、ネストなしの1階層のみ)
...
│ 💬 12   🔁 3.4万   ❤ 1.2万  ⤴ │
├─────────────────────────────┤
│  🙂 リプライ投稿者1 @user1     │ ← リプライ(order:2〜、PostCard)
│  返信本文が表示される           │
│  💬 1    🔁 2      ❤ 10      │
├─────────────────────────────┤
│  🙂 リプライ投稿者2 @user2     │ ← リプライ(order:3)
│  もう一件の返信本文             │
│  💬 0    🔁 0      ❤ 3       │
└─────────────────────────────┘
```
